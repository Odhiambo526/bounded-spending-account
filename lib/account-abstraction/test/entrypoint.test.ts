...
describe('EntryPoint', function () {
  ...
  describe('without paymaster (account pays in eth)', () => {
    ...
    describe('aggregation tests', () => {
      ...
      it('demonstrates deposit decreased during validation but beneficiary not compensated (pre-fix regression test)', async function () {
        // This test documents a discrepancy between entrypoint deposit accounting
        // and beneficiary compensation observed in the deployed EntryPoint contract:
        //  - validation phase decrements requiredPrefund from account deposit
        //  - execution phase (due to opInfos being zeroed) pays `collected = opInfo.prefund == 0`
        // Result: deposit decreases but beneficiary does not receive the corresponding funds.
        //
        // The test computes requiredPrefund locally from the userOp fields, so it does not
        // assume internal contract variables other than the prefund formula:
        //    requiredPrefund = (verificationGasLimit + callGasLimit + paymasterVerificationGasLimit +
        //                       paymasterPostOpGasLimit + preVerificationGas) * maxFeePerGas
        // (we use no paymaster here so paymaster* fields are zero)

        // isolate state changes
        const snap = await ethers.provider.send('evm_snapshot', [])
        try {
          // ensure the simple account has deposit to cover the prefund
          await simpleAccount.addDeposit({ value: ONE_ETH })

          const beneficiary = createAddress()
          const depositBefore = await entryPoint.balanceOf(simpleAccount.address)
          const beneficiaryBefore = await ethers.provider.getBalance(beneficiary)

          // choose explicit gas params so requiredPrefund is deterministic and small
          const verificationGasLimit = 100000 // 100k
          const callGasLimit = 100000         // 100k
          const preVerificationGas = 21000    // 21k
          const maxFeePerGas = BigNumber.from(1) // 1 wei to make prefund easy to compute
          const maxPriorityFeePerGas = BigNumber.from(1)

          // create a userOp that will execute a simple account call (use existing accountExecFromEntryPoint)
          // Build the actual UserOperation first to avoid any mismatch with helper defaults/normalization
          const userOpActual = await fillAndSign({
            sender: simpleAccount.address,
            callData: (await (async () => {
              const localCounter = await new TestCounter__factory(ethersSigner).deploy()
              const count = await localCounter.populateTransaction.count()
              const exec = await simpleAccount.populateTransaction.execute(localCounter.address, 0, count.data!)
              return exec.data!
            })()),
          ...
          // Compute requiredPrefund exactly like EntryPoint._getRequiredPrefund using the ACTUAL userOp fields
          const vglBN = BigNumber.from(userOpActual.verificationGasLimit)
          const cglBN = BigNumber.from(userOpActual.callGasLimit)
          const pvgBN = BigNumber.from(userOpActual.preVerificationGas)
          const pmVglBN = BigNumber.from(userOpActual.paymasterVerificationGasLimit ?? 0)
          const pmPostBN = BigNumber.from(userOpActual.paymasterPostOpGasLimit ?? 0)
          const maxFeeBN = BigNumber.from(userOpActual.maxFeePerGas)
          const requiredPrefund = vglBN.add(cglBN).add(pmVglBN).add(pmPostBN).add(pvgBN).mul(maxFeeBN)

          // sanity: depositBefore must be >= requiredPrefund
          expect(depositBefore).to.be.gte(requiredPrefund, 'test setup: deposit must cover required prefund')

          // pack and execute the actual userOp
          const userOpPacked = packUserOp(userOpActual)
          const tx = await entryPoint.handleOps([userOpPacked], beneficiary, { gasLimit: 10_000_000 })
          await tx.wait()

          const depositAfter = await entryPoint.balanceOf(simpleAccount.address)
          const beneficiaryAfter = await ethers.provider.getBalance(beneficiary)
          const depositDelta = depositBefore.sub(depositAfter)
          const beneficiaryDelta = beneficiaryAfter.sub(beneficiaryBefore)
          // optional debug
          // console.log('requiredPrefund=', requiredPrefund.toString(), 'depositDelta=', depositDelta.toString())

          // Observed-bug invariant assertions:
          // - deposit decreased during validation
          // - beneficiary did NOT receive compensation (pre-fix behavior)
          expect(depositDelta).to.be.gt(0)
          expect(beneficiaryDelta).to.equal(BigNumber.from(0))

          // Document: when fixed, test expectation should change to:
          // expect(beneficiaryDelta).to.equal(requiredPrefund)
          // expect(depositDelta).to.equal(requiredPrefund)
          // (keeps the invariants that deposit outflow equals beneficiary inflow)
        } finally {
          await ethers.provider.send('evm_revert', [snap])
        }
      })
